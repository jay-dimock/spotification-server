    ...
    .then((existingUser) => {
      ...
      if (existingUser !== null) {
        res
          .status(400)
          .json({
            errors: { id: { message: "A user already exists with this ID" } },
          });
      } else {
  ...
  User.findOne({ spotifyId: req.body.spotifyId })
    .then((user) => {
      if (user === null) {
        return res
          .status(400)
          .json({ errors: { email: { message: "Email was not found" } } });
      } else {
        return res.json({
          _id: user._id,
        });
...
module.exports.update = (req, res) => {
  User.findOneAndUpdate({ spotifyId: req.params.spotifyId }, req.body, {
    new: true,
  })
